...
> [!NOTE]
> В ТЗ [описаны шаги для интеграции фронтенда](https://zhukovsd.github.io/java-backend-learning-course/Projects/CurrencyExchange/#%D1%82%D0%B5%D1%81%D1%82%D0%BE%D0%B2%D1%8B%D0%B9-%D1%84%D1%80%D0%BE%D0%BD%D1%82%D0%B5%D0%BD%D0%B4).
> В этом проекте мы будем использовать чуть более простой способ интеграции - без скриптов и докера

1. В папку webapp добавлен фронтенд. [Репозиторий с фронтендом](https://github.com/zhukovsd/currency-exchange-frontend)
2. Файлы `launch-local-nginx.sh` и `.gitignore` нам не нужны их можно удалить
3. В файле `js/app.js` изменена переменная `host`, на хост текущего проекта
4. При запуске проекта по данному пути - http://localhost:8080/currency_exchange_rest_api_war/ должен отображаться
   фронтенд

> [!NOTE]
> В данном варианте интеграции нам не нужно реализовывать поддержку CORS на стороне бэкенда, т.к. фронтенд запущен на
> том же самом хосте, что и api

> [!IMPORTANT]
> Фронтенд написан в полном соответствии с ТЗ. 
> Если какие-то запросы не работают или данные не отображаются, то это повод проверить свою реализацию проекта на ошибки